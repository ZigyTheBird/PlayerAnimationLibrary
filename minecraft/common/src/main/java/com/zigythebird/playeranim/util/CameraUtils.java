...
public class CameraUtils {
    public static @Nullable AnimBone computeCamera(Camera camera) {
        PlayerAnimManager manager = ((IAnimatedPlayer)Minecraft.getInstance().player).playerAnimLib$getAnimManager();
        if (manager != null && manager.isActive()) {
            AnimBone bone = ((IAnimatedPlayer)Minecraft.getInstance().player).playerAnimLib$getAnimProcessor().getBone("head");
