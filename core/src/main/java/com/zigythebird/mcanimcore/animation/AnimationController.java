package com.zigythebird.mcanimcore.animation;

import com.zigythebird.mcanimcore.animation.keyframe.*;
import com.zigythebird.mcanimcore.animation.keyframe.event.CustomKeyFrameEvents;
import com.zigythebird.mcanimcore.animation.keyframe.event.data.CustomInstructionKeyframeData;
import com.zigythebird.mcanimcore.animation.keyframe.event.data.KeyFrameData;
import com.zigythebird.mcanimcore.animation.keyframe.event.data.ParticleKeyframeData;
import com.zigythebird.mcanimcore.animation.keyframe.event.data.SoundKeyframeData;
import com.zigythebird.mcanimcore.animation.layered.AnimationContainer;
import com.zigythebird.mcanimcore.animation.layered.AnimationSnapshot;
import com.zigythebird.mcanimcore.animation.layered.IAnimation;
import com.zigythebird.mcanimcore.animation.layered.modifier.AbstractFadeModifier;
import com.zigythebird.mcanimcore.animation.layered.modifier.AbstractModifier;
import com.zigythebird.mcanimcore.api.firstPerson.FirstPersonConfiguration;
...
import com.zigythebird.mcanimcore.bones.AdvancedAnimBone;
import com.zigythebird.mcanimcore.bones.AnimBone;
import com.zigythebird.mcanimcore.bones.PivotBone;
import com.zigythebird.mcanimcore.enums.AnimationFormat;
import com.zigythebird.mcanimcore.enums.PlayState;
import com.zigythebird.mcanimcore.enums.State;
import com.zigythebird.mcanimcore.enums.TransformType;
import com.zigythebird.mcanimcore.event.EventResult;
import com.zigythebird.mcanimcore.math.ModMatrix4f;
import com.zigythebird.mcanimcore.math.ModVector4f;
import com.zigythebird.mcanimcore.math.Vec3f;
import com.zigythebird.mcanimcore.molang.MolangLoader;
import com.zigythebird.mcanimcore.util.MatrixUtil;
...
public abstract class AnimationController implements IAnimation {
	...
	protected final Map<String, AdvancedAnimBone> bones = new Object2ObjectOpenHashMap<>();
	protected final Map<String, AnimBone> activeBones = new Object2ObjectOpenHashMap<>();
	...
	/**
	 * Initialize the bones for whatever you want to animate here.
	 */
	protected abstract void registerBones();
	...
	public void replaceAnimationWithFade(@NotNull AbstractFadeModifier fadeModifier, @Nullable RawAnimation newAnimation, boolean fadeFromNothing) {
		if (fadeFromNothing || this.isActive()) {
			if (this.isActive()) {
				...
				for (AnimBone bone : activeBones.values()) {
	...
	private void processCurrentAnimation(float adjustedTick, float seekTime, AnimationData animationData) {
		...
		if (adjustedTick >= animation.length()) {
			if (this.currentAnimation.loopType().shouldPlayAgain(animation)) {
				...
				if (nextAnimation == null) {
					this.animationState = State.STOPPED;
					this.currentAnimation = null;
					for (AdvancedAnimBone bone : this.bones.values()) {
		...
		for (Map.Entry<String, BoneAnimation> entry : animation.boneAnimations().entrySet()) {
			...
			AdvancedAnimBone bone = this.bones.get(entry.getKey());
	...
	protected void setupNewAnimation() {
		...
		resetEventKeyFrames();
		for (AdvancedAnimBone bone : bones.values()) {
		...
		for (Map.Entry<String, BoneAnimation> entry : currentAnimation.animation().boneAnimations().entrySet()) {
			if (bones.containsKey(entry.getKey())) {
				AdvancedAnimBone bone = bones.get(entry.getKey());
	...
	public AnimBone get3DTransformRaw(@NotNull AnimBone bone) {
		if (activeBones.containsKey(bone.getName())) {
			AnimBone bone1 = activeBones.get(bone.getName());
			if (this.currentAnimation != null && bone1 instanceof AdvancedAnimBone advancedBone) {
	...
	protected void internalSetupAnim(AnimationData state) {
		...
		for (BoneAnimationQueue boneAnimation : this.getBoneAnimationQueues().values()) {
			AnimBone bone = boneAnimation.bone();
        ...
        List<AnimBone> bones1 = new ArrayList<>(this.bones.values());
		for (AnimBone pivotBone : this.pivotBones.values()) {
			if (!parentsMap.containsValue(pivotBone.getName()))
				bones1.add(pivotBone);
		}

		for (AnimBone bone : this.bones.values()) {
	...
	protected void registerAnimBone(String name) {
		registerAnimBone(new AdvancedAnimBone(name));
	}
	 ...
	 */
	protected void registerAnimBone(AdvancedAnimBone bone) {
