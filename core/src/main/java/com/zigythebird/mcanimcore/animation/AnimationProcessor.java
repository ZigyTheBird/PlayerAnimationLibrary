...
import com.zigythebird.mcanimcore.enums.AnimationStage;
...
public abstract class AnimationProcessor {
	private final Map<String, AnimBone> bones = new Object2ObjectOpenHashMap<>();
	...
	/**
	 * Each AnimationProcessor must be bound to an entity.
	 * Register the bones needed to animate that entity here.
	 */
	public AnimationProcessor() {}

	/**
	 * This method is called once per render frame for each entity being rendered.
	 * <p>
	 ...
	 *
	 * @param AnimManager		The AnimManager instance being used for this animation processor
	...
	public void tickAnimation(AnimationStack AnimManager, AnimationData state) {
		AnimManager.getLayers().removeIf(pair -> pair.right() == null || pair.right().canRemove());
		for (Pair<Integer, IAnimation> pair : AnimManager.getLayers()) {
	 ...
	 * @return the bone
	 */
	public AnimBone getBone(String boneName) {
		return this.bones.get(boneName);
	}

	protected void registerAnimBone(String name) {
		registerAnimBone(new AnimBone(name));
	}
	 ...
	 */
	protected void registerAnimBone(AnimBone bone) {
		this.bones.put(bone.getName(), bone);
	}

	/**
	 * Get an iterable collection of the {@link AnimBone} currently registered to the processor
	 */
	public Collection<AnimBone> getRegisteredBones() {
		return this.bones.values();
	}

	/**
	 * {@link Animation} and {@link Animation.LoopType} override pair,
	 * used to define a playable animation stage for an entity
	 */
